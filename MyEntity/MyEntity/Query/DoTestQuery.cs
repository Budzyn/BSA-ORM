    ...
    public class DoTestQuery
    {
        ...
        public static void ShowDoTest()
        {
            using (var db = new MyContext())
            {
                ...
                var result = db.testdo.Where(item => item.Result > 22).GroupBy(s => s.UserID, (key, g) => new { City = key, Count = g.Count() });
                foreach (var test in result)
                {
                    Console.WriteLine(test);
                }
                Console.WriteLine("The result for each student");
                var rrr = (from DoTest in dts where (DoTest.Result > 0 && DoTest.Time > 0) select DoTest.UserID);
